...
// Ambil data siswa dan no HP dari pendaftar
$query = "
  SELECT 
    siswa.id_siswa,
    siswa.nama_lengkap,
    siswa.tempat_tgl_lahir,
    siswa.jenis_kelamin,
    siswa.status_siswa,
    pendaftar.nope
  FROM siswa
  JOIN pendaftar ON siswa.id_pendaftar = pendaftar.id_pendaftar
...
<body>
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 px-6">
            <h1 class="text-xl font-bold py-6 text-gray-800">Data Siswa</h1>
                        ...
                        ?>
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="py-2 px-4 flex gap-2">
                                    <a href="detail_data_siswa.php?id=<?= $row['id_siswa'] ?>" class="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-sm">Lihat</a>
                                    <form method="POST" action="update_status_formulir.php">
                                        <input type="hidden" name="id_siswa" value="<?= $row['id_siswa'] ?>">
                                        <button name="status_pending" value="diterima" class="bg-green-600 hover:bg-green-700 text-white px-4 py-1 rounded text-sm">Terima</button>
                                        <button name="status_pending" value="ditolak" class="bg-red-600 hover:bg-red-700 text-white px-4 py-1 rounded text-sm">Tolak</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endwhile; ?>
